...
dishRouter.route('/:id/comments')
        ...
        Dishes.findById(req.params.id)
            .then((dishes)=>{
                if(dishes != null){
                    ...
                    err = new Error('Dish of id '+req.params.id+' is not found');
                    err.status = 404;
                    return next(err);
                }  
            },(err)=>{next(err)})
    ...
    .put((req, res, next) => {
        res.statusCode = 403;
        res.end('PUT operation not supported on /comments/'+req.params.id+'/commets');
    })
    .delete((req, res, next) => {
        Dishes.findById(req.params.id)
            .then((dish)=>{
        ...
        Dishes.findById(req.params.id)
            .then((dish)=>{
                if(dish != null && dish.comments.id(req.params.commentId) != null){
                    ...
                    err = new Error('Comment of id '+req.params.commentId+' is not found');
                    err.status = 404;
                    return next(err);
                }
            },(err)=>{next(err)})
